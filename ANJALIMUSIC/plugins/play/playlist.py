...
from ANJALIMUSIC.utils import close_markup
from config import BANNED_USERS, SERVER_PLAYLIST_LIMIT
from ANJALIMUSIC import Carbon, app
from ANJALIMUSIC.utils.decorators.language import language, languageCB
from ANJALIMUSIC.utils.inline.playlist import (botplaylist_markup,
                                              get_playlist_markup,
                                              warning_markup)
...
async def add_playlist(client, message: Message, _):
    ...
    if "https://youtu.be" in query:
        ...
        from ANJALIMUSIC import YouTube
...
@app.on_callback_query(filters.regex("ANJALI_playlist") & ~BANNED_USERS)
@languageCB
async def add_playlist(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    videoid = callback_data.split(None, 1)[1]
